...
struct UnpairRepository {
   public static let shared = UnpairRepository()
   
   public func unpair(username: String, password:String, secretKey: String, transNumber: String, phoneNumber: String,tokenId: String, transType : String,success: @escaping (_ result: Unpair?) -> Void, failure: @escaping (_ failureMsg: FailureMessage) -> Void) {
       ...
       ApiService.shared.callAPI(serverURL: Constants.baseUrl,strURL: Constants.Path.finpayApiPhp, method: .post, headers: headers, parameters: requestBody, success: { response in
           do {
               if let data = response.data {
                   let result = try JSONDecoder().decode(Unpair.self, from: data)
                   success(result)
               }
           } catch {
