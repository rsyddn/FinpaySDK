...
struct UserBalanceRepository {
   public static let shared = UserBalanceRepository()
   ...
   public func upgradeAccount(username: String, password:String, secretKey: String, transNumber: String, phoneNumber: String,tokenId: String, success: @escaping (_ result: UpgradeAccount?) -> Void, failure: @escaping (_ failureMsg: FailureMessage) -> Void) {
       ...
       ApiService.shared.callAPI(serverURL: Constants.baseUrlCobrand,strURL: Constants.Path.getUserBalance, method: .post, headers: headers, parameters: requestBody, success: { response in
           do {
